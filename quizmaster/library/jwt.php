...
//        }
    ...
    public function getAuthorizationHeader() {
        ...
        // 1. Direct Authorization (works on some servers)
        if (isset($_SERVER['Authorization'])) {
            $headers = trim($_SERVER["Authorization"]);
        } 
        // 2. HTTP_AUTHORIZATION (Nginx, FastCGI, or .htaccess rewrite)
        else if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
            $headers = trim($_SERVER["HTTP_AUTHORIZATION"]);
        } 
        // 3. REDIRECT_HTTP_AUTHORIZATION (some Apache configurations)
        else if (isset($_SERVER['REDIRECT_HTTP_AUTHORIZATION'])) {
            $headers = trim($_SERVER["REDIRECT_HTTP_AUTHORIZATION"]);
        }
        // 4. Apache request headers (fallback)
        ...
        // 5. Check all $_SERVER keys for Authorization (case-insensitive search)
        if (empty($headers)) {
    ...
    public function getBearerToken() {
        ...
        if (!empty($headers)) {
            // Try to match Bearer token (case-insensitive, allow multiple spaces)
